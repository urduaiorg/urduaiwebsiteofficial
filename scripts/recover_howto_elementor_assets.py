...
REPORT_PATH = REPO_ROOT / "src" / "data" / "howto-elementor-assets.json"
...
def build_report() -> dict[str, object]:
    ...
    elementor_assets: dict[int, set[str]] = {post_id: set() for post_id in targets}
    ...
    for row in iter_insert_rows(SQL_DUMP_PATH, "wp_postmeta"):
        ...
        if post_id not in targets or meta_key != "_elementor_data" or not meta_value:
            continue

        elementor_assets[post_id].update(UPLOAD_PATTERN.findall(str(meta_value)))

    matched = []
    for post_id, meta in sorted(targets.items(), key=lambda item: item[1]["slug"]):
        uploads = sorted(elementor_assets.get(post_id, set()))
        matched.append(
            {
                "postId": post_id,
                "slug": meta["slug"],
                "title": meta["title"],
                "elementorUploads": uploads,
                "elementorUploadCount": len(uploads),
            }
        )

    posts_with_assets = sum(1 for item in matched if item["elementorUploadCount"] > 0)
    upload_total = sum(item["elementorUploadCount"] for item in matched)

    return {
        "generatedAt": datetime.now(timezone.utc).isoformat(),
        "summary": {
            "totalHowtoPosts": len(matched),
            "postsWithElementorUploads": posts_with_assets,
            "elementorUploadTotal": upload_total,
        },
        "targets": matched,
    }
